#!/usr/bin/env python3
"""
b_lqc_bounce_alpha_apply_v2_1.py
...
Outputs:
    ../grids/B_LQC_BOUNCE_alpha_grid_v2_1.csv
    ../vectors/B_LQC_BOUNCE_5d_vector_v2_1.csv
    ../summaries/B_ALPHA_APPLICATION_SUMMARY_v2_1.csv
    ../normalization_review/B_ALPHA_NORMALIZATION_REVIEW_v2_1.csv
...
DEFAULT_GRID = (
    ALPHA_ROOT / "grids" / "B_LQC_BOUNCE_alpha_grid_v2_1.csv"
)
DEFAULT_VECTOR = (
    ALPHA_ROOT / "vectors" / "B_LQC_BOUNCE_5d_vector_v2_1.csv"
)
DEFAULT_SUMMARY = (
    ALPHA_ROOT / "summaries" / "B_ALPHA_APPLICATION_SUMMARY_v2_1.csv"
)
DEFAULT_REVIEW = (
    ALPHA_ROOT
    / "normalization_review"
    / "B_ALPHA_NORMALIZATION_REVIEW_v2_1.csv"
)
...
def normalized_response(base: float, alpha: float, scale: float) -> float:
    if scale <= 0.0:
        return 0.0
    return abs(base * (alpha - 1.0)) / scale
...
def build_region_scales(
    ...
    for region in regions:
        rows = [r for r in intervals if r["normalization_region"] == region]
        scale_rows = pooled_near if region == "bounce_crossing" else rows
        if not scale_rows:
            raise ValueError(f"No intervals for normalization region: {region}")

        scales[region] = {
            "flow": robust_scale(to_float(r["signed_flow_gap_base"]) for r in scale_rows),
            "density": robust_scale(to_float(r["density_logchange_base"]) for r in scale_rows),
            "curvature": robust_scale(to_float(r["curvature_logchange_base"]) for r in scale_rows),
            "composition": robust_scale(to_float(r["composition_shift_base"]) for r in scale_rows),
            "margin": robust_scale(to_float(r["margin_change_base"]) for r in scale_rows),
        }
    return scales
...
def compute_vector(
    ...
    notes = (
        "Dataset B full-path alpha vector with five-region conditioned normalization; bounce-safe signed-flow coordinate "
...
def build_summary(
    ...
    return {
        ...
        "collapse_onset_radius": vector["collapse_onset_radius"],
        "script_version": SCRIPT_VERSION,
        "status": "complete_full_bounce_path",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply the v2 alpha workflow to the full Dataset B LQC bounce path."
        )
    )
