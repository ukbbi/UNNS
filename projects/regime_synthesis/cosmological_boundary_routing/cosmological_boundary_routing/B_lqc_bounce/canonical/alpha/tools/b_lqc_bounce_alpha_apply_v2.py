...
Outputs:
    ../grids/B_LQC_BOUNCE_alpha_grid_v2.csv
    ../vectors/B_LQC_BOUNCE_5d_vector_v2.csv
    ../summaries/B_ALPHA_APPLICATION_SUMMARY_v2.csv
    ../normalization_review/B_ALPHA_NORMALIZATION_REVIEW_v2.csv
...
DEFAULT_GRID = (
    ALPHA_ROOT / "grids" / "B_LQC_BOUNCE_alpha_grid_v2.csv"
)
DEFAULT_VECTOR = (
    ALPHA_ROOT / "vectors" / "B_LQC_BOUNCE_5d_vector_v2.csv"
)
DEFAULT_SUMMARY = (
    ALPHA_ROOT / "summaries" / "B_ALPHA_APPLICATION_SUMMARY_v2.csv"
)
DEFAULT_REVIEW = (
    ALPHA_ROOT
...
def build_grid_rows(
    ...
    scales = {
        "flow": robust_scale(
            to_float(r["signed_flow_gap_base"]) for r in intervals
        ),
        "density": robust_scale(
            to_float(r["density_logchange_base"]) for r in intervals
        ),
        "curvature": robust_scale(
            to_float(r["curvature_logchange_base"]) for r in intervals
        ),
        "composition": robust_scale(
            to_float(r["composition_shift_base"]) for r in intervals
        ),
        "margin": robust_scale(
            to_float(r["margin_change_base"]) for r in intervals
        ),
    }
    ...
    for interval in intervals:
        ...
        for alpha in alphas:
            ...
            flow_response = normalized_response(
                flow_base, alpha, scales["flow"]
            )
            ...
            grid.append(
                {
                    ...
                    "alpha_status": (
                        "valid" if instability == "no" else "unstable"
                    ),
...
def compute_vector(
    ...
    notes = (
        "Dataset B full-path alpha vector; bounce-safe signed-flow coordinate "
...
def main() -> int:
    ...
    grid_rows = build_grid_rows(intervals, alphas)
    vector = compute_vector(grid_rows, intervals, alphas)
    review = compute_review(grid_rows, vector)
