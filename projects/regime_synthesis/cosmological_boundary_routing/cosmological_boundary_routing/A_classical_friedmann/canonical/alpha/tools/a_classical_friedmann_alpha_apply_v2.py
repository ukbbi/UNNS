...
Dataset A alpha-application / deformation-grid constructor, VERSION 2, for
COSMOLOGICAL BOUNDARY ROUTING.

This script adapts the alpha-grid method used in the recent Stellar Boundary
Dynamics workflow to the validated classical Friedmann contraction trajectory.
...
5. computes structural response and persistence from active channels only;
6. retains the provisional margin channel as diagnostic-only;
7. exports the established 5D rigidity vector;
8. writes a normalization-review record and run summary.
...
Methodological status:
    VERSION 2. The provisional boundary-margin channel is retained in the grid
    for diagnosis only and does not contribute to structural_response,
    phase_persistence_score, instability_flag, or collapse_onset_radius.
    The direct STRUC-PERC-I and STRUC-I results remain separate.
...
OBJECT_ID = "A_CLASSICAL_FRIEDMANN_V2"
SCRIPT_VERSION = "2.0.0"
OBJECT_NAME = "Classical Friedmann singular approach"
DOMAIN = "classical-friedmann-contraction"
...
DEFAULT_GRID = DEFAULT_ALPHA_ROOT / "grids" / "A_CLASSICAL_FRIEDMANN_alpha_grid_v2.csv"
DEFAULT_VECTOR = DEFAULT_ALPHA_ROOT / "vectors" / "A_CLASSICAL_FRIEDMANN_5d_vector_v2.csv"
DEFAULT_SUMMARY = (
    DEFAULT_ALPHA_ROOT
    / "summaries"
...
def build_base_intervals(
    ...
    for index in range(len(trajectory_rows) - 1):
        ...
        h_signed_left = to_float(left.get("H_signed_km_s_Mpc"))
        h_signed_right = to_float(right.get("H_signed_km_s_Mpc"))
        ...
        interval = {
            ...
            "H_signed_mid_km_s_Mpc": 0.5 * (h_signed_left + h_signed_right),
...
def build_grid_rows(
    ...
    for interval in base_intervals:
        ...
        for alpha in alphas:
            ...
            # VERSION 2 active weighting.
            # The provisional margin channel remains diagnostic-only and does
            # not contribute to structural response or instability.
            structural_response = (
                0.35 * gap_response
            ...
            grid_rows.append(
                {
                    ...
                    "trajectory_direction": interval["trajectory_direction"],
                    "H_signed_mid_km_s_Mpc": fmt(
                        to_float(interval["H_signed_mid_km_s_Mpc"])
                    ),
                    "alpha": fmt(alpha),
...
def compute_vector(
    ...
    notes = (
        "Dataset A classical-contraction alpha vector, version 2; "
        "trajectory order is preserved from a=1 toward a=1e-8; "
        "signed H remains negative; "
        "mean_GR and var_GR use normalized ln(E)-gap response; "
        "anisotropic_persistence is variance of mean structural response "
        "across lambda-, matter-, and radiation-dominated contraction intervals; "
        "active weights are gap=0.35, density=0.30, curvature=0.20, "
        "composition=0.15; provisional margin response is diagnostic-only; "
...
def compute_normalization_review(
    ...
    active_ratios = {
        "density": ratios["density"],
        "curvature": ratios["curvature"],
        "composition": ratios["composition"],
    }
    maximum_active_ratio = max(active_ratios.values(), default=0.0)

    anisotropy = to_float(vector["anisotropic_persistence"])
    active_scale_review_needed = (
        maximum_active_ratio >= 10.0 or anisotropy >= 100.0
    )
    margin_diagnostic_warning = ratios["margin"] >= 10.0

    review_status = (
        "review_required"
        if active_scale_review_needed
        else "comparable_active_channels"
    )

    notes = (
        "version 2 review; only active channels can trigger scale rejection; "
        "margin response is retained as diagnostic-only and may generate a "
        "warning without invalidating the active-channel vector; "
    ...
    return {
        ...
        "active_scale_review_needed": "yes" if active_scale_review_needed else "no",
        "margin_diagnostic_warning": "yes" if margin_diagnostic_warning else "no",
...
def build_summary(
    ...
    return {
        ...
        "script_version": SCRIPT_VERSION,
        "status": "complete_direction_preserving",
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Apply VERSION 2 of the alpha-grid workflow to the validated "
            "Dataset A classical Friedmann contraction trajectory."
        )
    )
    parser.add_argument(
        "--input",
        ...
        help="Validated classical Friedmann contraction trajectory CSV.",
    )
    parser.add_argument(
        "--grid-output",
        type=Path,
    ...
    print("Margin channel: diagnostic-only")
