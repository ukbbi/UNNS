...
Let $\mathcal{R}$ be the class of admissible UNNS recursive structures.  
For any $R \in \mathcal{R}$, Operator XII performs the collapse
...
- **Collapse invariance**
  ...
  $$
  \mathbf{XII}(R) = \mathbf{XII}(S_0(R)).
  $$

- **Non-transport**

  $S_0(R)$ is inert and non-propagating.
...
- **Transport**
  ...
  $S_{\tau}(R) \in \mathrm{Trans}(\mathcal{R})$.

- **Non-idempotence**

  $$
  S_{\tau}(S_{\tau}(R)) = \tau^2 S_0(R).
  $$
...
- **Local restart:** from $S_0(R)$  
- **Transported restart:** from $S_{\tau}(R)$
